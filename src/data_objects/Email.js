...
export class Email {
    ...
    add_random_tasks() {
        ...
        if (Math.random() < 0.5) {
            ...
            var source_indexes = {
                start: rand_int(0, this.get_text().length - 1),
            };
            source_indexes['end'] = rand_int(
                source_indexes.start + 1,
    ...
    get_receivers() {
        let result = new Set();
        for (let contact of this.get_recipients()
            .concat(this.get_ccs())
            .concat(this.get_bccs())) {
            result.add(contact);
        }
    ...
    get_bccs() {
        return this.email['bccRecipients'].map((c) =>
