...
export class Thread {
    constructor(id, email_objects) {
        this.id = id;
        this.emails_dict = email_objects.reduce((dict, e) => {
            dict[e.get_id()] = e;
            return dict;
        }, {});
    }
    ...
    get_emails(folder_id = Thread.SELECTED_FOLDER_ID) {
        ...
        if (folder_id) {
            emails = emails.filter((email) =>
                email_folder_filter(email, Email.FOLDER_MAPPINGS, folder_id)
            );
        }
        return emails;
    }
    ...
    delete_all(dispatcher) {
        ...
        dispatcher(
            this.id,
            emails.map((email) => email.get_id())
        );
    }
}

Thread.group_functions[PRIORITY_KEY] = (thread) =>
    thread.get_priority(Thread.SELECTED_FOLDER_ID);
...
Thread.sort_functions[PRIORITY_KEY] = (a, b) =>
    a.get_priority(Thread.SELECTED_FOLDER_ID) -
    b.get_priority(Thread.SELECTED_FOLDER_ID);

Thread.sort_functions[TIME_KEY] = (a, b) =>
    b.get_date().valueOf() - a.get_date().valueOf();
