...
export default class EmailThread extends Component {
  ...
  has_unread() {
    return this.props.thread.has_unread();
  }
  ...
  render() {
    ...
    const stamp = sender
      ? EmailStamp([sender.get_icon()], selected_email.date, sender.get_name())
      : null;
    return (
      <div
        className={this.get_style()}
        onClick={() => this.props.handle_select(this.props.id)}
      >
        {stamp}
        <EmailTextArea
          isUnread={this.has_unread()}
          ...
          external_on_task_portal_close={() =>
            this.setState({ show_task_portal: false })
          }
...
function ThreadLabels(props) {
  ...
  const num_tasks = tasks.filter(
    (t) => !t.is_done() && t.approved() && t.thread_id === props.thread_id
  ).length;
  const option_names = [
    "Set as task",
    "Change priority",
  ...
  let options = option_names.map((n) => {
    return { name: n };
  });
  options.filter((o) => o.name === "Delete")[0].action = () => {
    thread.delete_all((t_id, e_ids) => dispatch(DeleteEmails(t_id, e_ids)));
  };
  options.filter((o) => o.name === "Set as task")[0].action =
    props.show_task_portal;
  const email_id = thread.get_emails()[0].get_id();

  options.filter((o) => o.name === "Mark as read")[0].action = () =>
    thread.mark_all_read();
  options.filter((o) => o.name === "Change priority")[0].action = () =>
    thread.set_priority((thread.get_priority() + 1) % 3);
  options.splice(
    2,
    0,
    ...["reply", "reply_all", "forward"].map((type) =>
      build_email_action_button(dispatch, type, email_id)
    )
  );
  return (
