  ...
  return thread && props.show ? (
    <div className={style}>
      <SimpleBar className="SimpleBar_RightDivision">
        <Tasks
          tasks={tasks.filter((t) => t.thread_id === thread.id && t.approved())}
          thread={thread}
        />
  ...
  const body = empty ? (
    ...
    <ul>
      {props.tasks.map((task) => {
        ...
        return (
          <li key={task.id} className={get_priority_style(task.priority)}>
            <span
            ...
            <span onClick={() => props.on_click(task.id)} className="task_text">
              {task.text}
            </span>
          </li>
        );
