...
export function FileAttachments(attachments) {
  ...
  const attachemnts_for_display = attachments.map((a) => (
    <AttachmentDisplay attachment={a} />
  ));
  return (
    <div className="Container">
      <h4>Attached Files</h4>
      <div className="AttachmentsDisplay">{attachemnts_for_display}</div>
    </div>
  );
}
...
export function AttachmentDisplay(props) {
  ...
  const preview_attachment = () => {
    set_visible(true);
    const url = download_attachment(
      props.attachment.email_id,
      props.attachment.id,
      true
    );
  };
  return (
    <div
      className="TitledImage"
      onClick={() =>
        download_attachment(props.attachment.email_id, props.attachment.id)
      }
    >
