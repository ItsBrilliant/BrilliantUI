...
import PriorityOptions from '../misc/PriorityOptions.js';
import { EmailComposer } from '../misc/EmailComposer.js';
...
import { TaskOwnerSelect } from './TaskOwnerSelect';
...
function TaskConversation(props) {
    const messages = props.task.messages.map((message) => {
        const sender = Contact.create_contact_from_address(
    ...
    return (
        <div className="TaskConversation">
            ...
            <EditingTextArea
                ...
                start_value=""
                handle_enter={(val) =>
                    Task.update_task(props.updater, props.task, 'add_message', [
                        val,
                    ])
                }
            />
