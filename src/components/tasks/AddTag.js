...
export default function AddTag(props) {
    ...
    const component =
        <AddTagStyle location={location}>
            <TagChips on_items_change={props.on_items_change} items={props.task.tags}></TagChips>
        </AddTagStyle>
    return (
        <>
            <button className="add_tag" onClick={open_portal}>{props.task.tags.length} tags</button>
            <GeneralPortal visible={visible} component={component} handle_close={() => set_visible(false)} />
        </>
    )
}
