...
export class MsgrahpAdaptor extends JsonAdaptor {
    constructor(internal_delete_function) {
        super();
        this.delete_event = internal_delete_function;
    }
    insert() {
    ...
    update() {
        ...
        event_action(event, event.id).then(
            (res) => (arguments[2].id = res.data.id)
        );
        return original;
    }
    ...
    remove() {
        ...
        this.delete_event(arguments[2].id);
...
export function place_priority(events) {
    let appointments = Array.from(
        document.querySelectorAll('div.e-appointment')
    );
    ...
    return (
        <Fragment>
            <input
                ...
                style={{ display: 'none' }}
            />
            <EmailChips on_items_change={my_set_items} items={items} />
        </Fragment>
    );
}
...
export function build_event(schedule_event) {
    ...
    event.id =
        schedule_event.priority === 'suggested' ? undefined : schedule_event.id;
    event.start = convert_time_to_graph(
        schedule_event.start,
        schedule_event.StartTimeazne
    );
    event.end = convert_time_to_graph(
        schedule_event.end,
        schedule_event.EndTimezone
    );
    event.location = { displayName: schedule_event.location };
    event.attendees = !schedule_event.participants
        ? []
        : schedule_event.participants.split(',').map((a) => {
