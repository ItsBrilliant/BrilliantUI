...
import {
  ...
  update_and_send,
  update_draft,
  append_email_attachments,
  delete_attachment,
} from "../backend/Connect.js";
import { sleep } from "../utils.js";
import {
  build_email_from_composer,
  get_recipient_addresses_from_email,
} from "./email_compuser_utils.js";
...
export function EmailComposers() {
  ...
  const composers = composers_state.ids.map((n) => (
    <div
      className={focused === n ? "on_top" : undefined}
      onClick={(e) => set_focus(n)}
    >
      <EmailComposer
...
export function EmailComposer(props) {
  ...
  useEffect(() => process_attributes(), [props.email_attributes.email_id]);
  ...
  const handle_send = (html) => {
    props
      .send(to, subject, html, cc, bcc, file_buffers, files, dest_id)
      .then((res) => {
        if (res) {
          handle_close(true);
        }
  ...
  async function process_attributes() {
    ...
    try {
      if (
        ["reply", "reply_all", "forward"].includes(attributes.composer_type)
      ) {
        const message = await create_based_draft(
          attributes.email_id,
          attributes.composer_type
        );
        var email_object = new Email(message);
        if (attributes.composer_type === "forward") {
          append_email_attachments([email_object]).then((res) =>
            set_files(email_object.get_attachments())
          );
        }
    ...
    try {
      if (!email_was_sent) {
        ...
        if (need_to_save_draft(to, subject, html_content, cc, bcc)) {
          const current_composer_email = build_email_from_composer(
            to,
            subject,
            html_content,
            cc,
  ...
  const email_content = (
    <EmailContent
      ...
      set_files={(e) =>
        my_set_files(e, set_files, my_set_buffers, my_set_progress, props.id)
      }
  ...
  if (props.only_content) {
    return <div className="EmailComposer">{email_content}</div>;
  } else {
    return (
      <Draggable
        handle=".EmailComposer"
        cancel=".EmailContent"
        defaultPosition={{ x: 50 * props.id, y: 0 }}
      >
        <div className="EmailComposer">
          <ComposeHeader
            on_close={(e) => handle_close(false)}
...
function ComposeHeader(props) {
  return (
    <div className="header">
      ...
      <button className="delete" onClick={props.on_close}>
        &times;
      </button>
    </div>
  );
...
export function my_set_files(
  ...
  set_files((old_files) => [
    ...old_files,
    ...new_files.filter(
      (f) => !old_files.map((of) => of.name).includes(f.name)
    ),
  ]);
...
function scroll_to_files(id) {
  const simplebar = document.querySelector(
    "#EmailContent" + id + " .simplebar-content-wrapper"
  );
...
export function EmailSendMessage(props) {
  ...
  return (
    <div className={"email_sent_message" + style}>
      ...
      <span className="email_sent_undo" onClick={props.on_undo}>
        Undo
      </span>
    </div>
  );
