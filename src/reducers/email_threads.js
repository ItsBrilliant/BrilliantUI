...
export const EmailThreadsReducer = (state = {}, action) => {
    if (action.type === "EXPAND_THREADS") {
        ...
        var new_state = Object.assign({}, state);
        const wanted_email_ids = current_email_ids.filter(i => !action.email_ids.includes(i));
        if (wanted_email_ids.length === 0) {
            delete new_state[action.thread_id]
        } else {
            new_state[action.thread_id] = new Thread(action.thread_id, wanted_email_ids.map(i => current_emails_dict[i]));
        }
        return new_state;
    }
